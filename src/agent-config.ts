...
- 需要组织的业务背景、历史决策、环境配置、踩坑记录等隐性知识时，优先 grep / 读取该目录下的 Markdown。
- 产生了值得沉淀、对他人有用的新知识时，直接在该目录写入 / 编辑 Markdown 即可；保存即同步，无需 git add/commit/push。
- 需要进行大范围改动前先运行 \`knowbase pause\`，完成后 \`knowbase resume\`，避免半成品被自动提交。
${BLOCK_END}`;
}
